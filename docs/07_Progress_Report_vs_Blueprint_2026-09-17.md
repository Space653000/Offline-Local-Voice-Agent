...
### 2026-09-18 第二十一次更新：使用者對3個SOP項目做出決定，喚醒詞開關已實作完成（取代8小時測試）

使用者對`docs/08`的3個SOP項目回覆：①端點延遲取捨→選B（縮短等待，參考「其他雲端AI都預設1秒」）②8小時誤觸發測試→**取消，改用主介面喚醒開關（預設關閉）這個更根本的解法**③P5視覺備援→選B（較小VL模型，跟文字LLM同時常駐）。

**這次先完成②**（技術細節見`docs/06`「喚醒詞開關」一節）：新增`src/wake_setting.py`獨立輕量模組管理開關狀態（預設關閉、讀取失敗一律當關閉），`listen_loop.py`的`process_chunk()`在開關關閉時完全不呼叫喚醒詞預測模型（不是跑了忽略結果），`companion.html`最上面新增明顯的勾選框。新增安全測試`src/test_wake_toggle_safety.py`，用真實合成喚醒詞音檔驗證：開關關閉時零事件觸發、開關打開時功能正常。用瀏覽器實測勾選框真的能正確寫入/讀取設定檔。9個回歸測試全部通過。

這個設計直接取代原本規劃的8小時無人值守測試需求——不需要再統計誤觸發率多低，因為不管多低，只要開關沒打勾，誤觸發都不可能造成任何實際動作。

①③正在處理中，會分別在完成後補上這份報告的後續更新。

### 2026-09-18 第二十二次更新：端點偵測延遲調整完成（選項B），真實數字：快了約446ms，離1秒還有距離

`docs/08`§4.1的決定（選項B，縮短安全邊際等待）已完成（技術細節見`docs/06`對應章節）。調整`config/runtime.yaml`的兩個端點偵測參數，安全邊際總和從1.12秒降到0.72秒。調整前後各真實跑3次`run_voice_latency_test.py`（真的模擬麥克風即時串流節奏），結果：Wake→ASR總延遲從平均5603ms降到5157ms，快了約446ms，改善幅度跟參數調整的理論預期吻合。

誠實結論：確實變快了，但離1.5秒藍圖目標、離使用者提到的「雲端AI約1秒」都還有相當距離——純ASR轉錄本身就要1.5~1.7秒，這是這個本機whisper.cpp medium模型架構下的物理限制，不是安全邊際的問題，要再更快需要換更快/串流式的ASR模型，是比調參數更大的架構改動。是否增加誤切話尾的真實風險沒有用真人測試驗證，這點誠實記錄為未驗證。

### 2026-09-18 第二十三次更新：P5視覺備援真實下載+接線測試完成（方案B），關鍵發現：grounding精確度不夠

使用者同意下載`Qwen/Qwen3-VL-8B-Instruct-GGUF`（官方Qwen帳號，約5.78GB）並真實測試（完整報告見`progress/p5_vision/EVALUATION_REPORT.md`，技術細節見`docs/06`對應章節）。

真實結果：①第一次啟動CUDA OOM（context size預設值太大），加`-c 4096`修好 ②跟文字LLM同時常駐成功，總VRAM用量10.6~10.9GB，方案B的核心目標達成 ③場景理解準確（正確描述真實桌面截圖內容） ④🔴精確定位（bounding box）測出來不夠準——要求定位「開始按鈕」，回答格式正確但座標偏移螢幕高度6~7%，落在空白區域，而且confidence高達0.98（自信講錯，比老實不確定更危險），用截圖標紅框直接證明。

誠實結論：這是換小模型的合理代價，不是bug。建議這個能力先限制在「不需要精確座標」的粗略判斷場合，不直接信任座標去自動點擊——這比藍圖原本寫的「Vision不直接控制Mouse」原則要求更嚴格（連高confidence都不能直接信）。

### 2026-09-19 第二十四次更新：P5 grounding問題追加測試——業界標準修法（兩階段裁切放大）也解不掉

使用者對P5的grounding不準問了「解不掉？」，追加測試業界常見的修法：先粗略定位、裁切放大、再精確定位（完整資料見`progress/p5_vision/EVALUATION_REPORT.md`第6節、`progress/p5_vision/evidence/two_stage_attempt_result.json`）。

結果：**沒有解決問題**。裁切放大後畫面裡按鈕清楚可見，但模型第二階段給的座標超出了放大圖片本身的邊界，比第一次直接在全圖上定位錯得更明顯，confidence仍然0.99。誠實結論：問題不是「畫面太大太複雜」，是這個8B小模型本身的空間grounding能力不夠，跟畫面難度關係不大，這個方向的修法無效。剩下的真實選項是換回大模型（方案A）、完全不信任這個模型的座標只做粗略判斷、或暫時擱置P5——留給使用者決定。

### 2026-09-19 第二十五次更新：🔴重大修正——P5 grounding問題找到真正根因，是座標系統理解錯誤，不是模型能力不夠，已修好

上一次更新（第二十四次）記錄「業界標準修法（兩階段裁切放大）也解不掉grounding問題」，**這個結論是錯的**，這次抓到並修正了真正的根因（完整過程見`progress/p5_vision/EVALUATION_REPORT.md`第7節）。

使用者授權「下載30B自己判斷，不好用就刪」，換上原本blueprint指定的30B大模型測試——**結果30B給出幾乎一模一樣錯誤的座標**，這個「兩個不同尺寸模型犯一模一樣的錯」才是真正的關鍵線索：不是模型能力問題。追查發現模型內部對截圖做了resize（自己回報看到的尺寸跟實際不符），而且回答的座標其實是0~1000的正規化座標（Qwen-VL訓練慣例），不是prompt裡要求的像素座標——模型沒有照prompt的話做，還是用它訓練時習慣的方式回答。

修正prompt明確要求0~1000正規化座標、程式碼自己換算成像素座標後，**8B小模型（不需要換30B）在兩個不同UI元素測試都準確定位**。已刪除下載的30B模型檔案（19.3GB）。

**誠實記錄的教訓**：換更大的模型表面上看起來該更準，但這次真正的問題完全不是模型能力，換大模型不但沒解決問題，還多下載19.3GB、多等近30秒才發現真正原因——VLM輸出「看起來合理但對不上」的座標時，應該先懷疑座標系統假設，不要先跳去找更大的模型。這也是這份報告第二次記錄「先前的分析結論是錯的」（第一次是P3的UWP相容性誤判），跟那次一樣，是誠實記錄不是悄悄抹掉重寫。

P5目前狀態：技術上已經沒有阻礙，可以開始真的接進Executor流程，用現有的8B模型即可。
