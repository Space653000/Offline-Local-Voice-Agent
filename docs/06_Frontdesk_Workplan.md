...
## 新增memory_op（參考雲端AI助理的「記憶」功能），抓到並修正一個中文模糊比對的真實bug

`user_preferences`這張表跟`set_preference()`/`get_preference()`兩個函式其實在更早的階段就已經存在，但完全沒有任何地方呼叫過——跟這次盤點之前`conversation_log`是同一種「表跟函式都在，就是沒接上」的落差。這次補上完整對外接口`memory_op`（`remember`/`recall`/`forget`），參考ChatGPT等雲端AI助理的「記憶」功能，讓使用者能用講的/打字告訴助理「記住我喜歡...」，下次問「我喜歡什麼」能真的答得出來——資料完全存在這台機器本機的sqlite檔案，不會傳到任何外部服務。`remember`/`forget`設L1（容易復原：記錯了再講一次覆蓋過去、或講「忘記」刪掉），`recall`是L0純查詢。

🔴 **第一次跑真實NL測試就抓到一個bug**：「幫我記住我喜歡簡短扼要的回答」讓LLM把`key`填成「喜歡簡短回答」，但下一輪使用者問「我之前跟你說過我喜歡什麼樣的回答」，LLM這次填的`key`變成「喜歡的回答」——兩次用詞不完全一樣（中間多一個「的」字），單純的精確比對或子字串包含比對都抓不到，導致明明記住過卻回報「沒有記住過這件事」。**中文沒有天然的分詞邊界，這是LLM每次生成自由文字`key`本身就會有的變異，不是特例**。修法：`recall`比對不到精確key時，改用字元集合的Jaccard相似度（兩個詞共用了多少字/總共出現過幾個不同的字）當模糊比對依據，超過0.4的相似度門檻才採用，避免誤配到完全不相關的記憶。修好後重測：「喜歡的回答」正確配對到「喜歡簡短回答」；同時驗證真的問一個完全沒記住過的事情（「完全不存在的東西」）會正確回報「沒有記住過」，不會誤配。

跑完整回歸測試（8個測試檔）全部通過，測試用的偏好資料測完已清除。

## 喚醒詞開關：取代8小時無人值守測試的根本解法（使用者2026-09-18明確決定）

使用者對`docs/08`§4.2「喚醒詞8小時誤觸發率測試」的回覆不是「同意測」，而是**取消這個測試，改成更根本的解法**：主介面加一個喚醒詞開關，預設關閉，關閉時電腦不能在使用者不知情的狀況下自己做任何小事。這個決定比原本SOP項目本身更好——與其統計「多久誤觸發一次」然後接受某個誤觸發率、期望它夠低，不如直接讓誤觸發本身不可能造成任何動作，徹底消除這個疑慮，不需要靠統計數字說服使用者放心。

**實作**：

- **`src/wake_setting.py`（新檔案）**：獨立的輕量模組，只負責讀寫一個小JSON檔案(`console/wake_word_setting.json`)記錄開關狀態。故意設計成不依賴`openwakeword`/`whisper`這些重量級套件，讓`serve.py`（本身不需要語音模型）也能低成本import。**預設值是關閉**——檔案不存在、或讀取失敗，一律當作關閉，不能因為設定檔案遺失/損毀就意外變成開啟。
- **`listen_loop.py`的`process_chunk()`**：🔴安全關鍵的一行——開關關閉時，`self.wake_model.predict(chunk)`根本不會被呼叫，不是「跑了但忽略結果」。這樣才是真正的保證，不是留一個「萬一判斷邏輯哪裡漏接還是會觸發」的後門。
- **這個開關獨立於`memory_op`那套一般記憶機制**，LLM沒有任何工具能直接改動它（不在`full_pipeline.py`的`TOOL_ENUM`裡），只有`companion.html`最上面那個勾選框（使用者手動操作）能改變這個值，避免被一般對話「不小心」講出什麼觸發改動。
- **`companion.html`**：最上面新增一個明顯的勾選框卡片（不是塞在設定選單裡），預設不勾選，文字清楚說明「沒打勾＝完全不會主動聆聽或執行任何動作，只能用下面打字」。勾選狀態會即時同步到後端，也會反映在`live_state.json`裡（讓畫面能顯示目前語音助理程式看到的真實開關狀態）。新手引導流程也更新，第一步就講清楚這個開關預設是關的。

**真實測試（安全關鍵功能，測得特別仔細）**：

- 新增`src/test_wake_toggle_safety.py`，用跟`test_listen_loop_simulated.py`一樣的真實合成喚醒詞音檔測試：開關關閉時，餵進去完整的「嗨小助理」音檔，**確認完全零事件觸發**；開關打開時，同一段音檔能正常喚醒並聽完指令，證明開關本身沒有把既有功能弄壞。
- 用瀏覽器工具實際操作`companion.html`的勾選框（不是只呼叫底層函式），確認點擊後`wake_word_setting.json`真的被寫入正確的值、畫面文字即時更新、再點一次能正確切回關閉狀態。
- 跑完整回歸測試（9個測試檔，含新加的安全測試）全部通過。

## 端點偵測延遲調整（使用者2026-09-18決定選項B），真實重新量測

使用者對`docs/08`§4.1的決定是選項B（縮短安靜確認等待），並提到「其他雲端AI都預設1秒」當參考基準。調整`config/runtime.yaml`：`end_of_speech_silence_chunks`從10（0.8秒）降到6（0.48秒），`quiet_chunks_to_confirm_wake_ended`從4（0.32秒）降到3（0.24秒）——兩段安全邊際加總從1.12秒降到0.72秒，刻意不是砍到接近0（P1階段踩過坑：緩衝期太短會把使用者話還沒講完就誤判成講完、切斷話尾），是折衷後的中等幅度調整。

**真實重新量測（`progress/p4_kpi_measurement/run_voice_latency_test.py`，調整前後各跑3次，方法論跟原本第一次量測一致：真的用`time.sleep()`模擬麥克風即時串流節奏，不是把音訊陣列一次全塞進迴圈）**：

| 量測區段 | 調整前（3次平均） | 調整後（3次平均） | 差異 |
|---|---|---|---|
| wake_detected → utterance_end（安全邊際那一段，受這次調整直接影響） | 3879ms | 3553ms | **快了約326ms** |
| utterance_end → asr_result（純ASR轉錄時間，理論上不受這次調整影響） | 1724ms | 1605ms | 差異屬於run跟run之間的正常雜訊範圍，不是調整造成的 |
| Wake→ASR總延遲 | 5603ms | 5157ms | **快了約446ms**（5.6秒→5.16秒） |

**誠實結論**：確實變快了，而且改善幅度（~330~450ms）跟參數調整的理論預期（~400ms）吻合，不是雜訊。但**離藍圖1.5秒的目標、離使用者提到的「雲端AI約1秒」參考值，都還有相當大差距**——純ASR轉錄本身就要1.5~1.7秒，這個架構下光是「聽懂你在講什麼」這件事，物理上就已經超過1秒了，不是安全邊際的問題。已經把安全邊際能縮的部分縮了，接下來如果還要更快，需要換更快的ASR模型或改用串流式ASR（一邊聽一邊轉錄，不用等使用者講完整句話才開始），這是比調參數更大的架構改動，不在這次選項B的範圍內。
...
**誠實結論（2026-09-18初版，已被下方2026-09-19的修正推翻）**：~~這是換小模型（省顯示卡記憶體）的合理代價——grounding精確度通常隨模型尺寸下降，不是實作有bug~~。測試用的伺服器已關閉，模型檔案留在`progress/p5_vision/models/`（已加進`.gitignore`，不會被commit進repo）。

**補充實驗（使用者問「解不掉？」之後追加測試，這一版結論也已被推翻）**：試了業界常見的「先粗略定位→裁切放大→再精確定位」兩階段修法。**當時結論是「沒有解決問題」**，完整原始資料見`progress/p5_vision/evidence/two_stage_attempt_result.json`。

## 🔴 2026-09-19重大修正：真正的根因是座標系統理解錯誤，不是模型能力問題，已修好，不需要換大模型

使用者授權「先下載30B再自己判斷，不好用就刪，好用就留」，換上原本blueprint指定的30B大模型（`Qwen3-VL-30B-A3B-Instruct`，18.6GB+712MB mmproj）測試——**結果30B給出幾乎一模一樣錯的座標**（`[10,962,35,991]` vs 8B的`[0,965,24,999]`）。兩個不同尺寸的模型犯一模一樣的錯，這個現象才是真正抓到問題不在模型能力的關鍵線索，完整追查過程跟修正方法詳見`progress/p5_vision/EVALUATION_REPORT.md`第7節，重點摘要：

- 直接問模型「這張圖片你看到的寬度跟高度是多少」，回答「1701x900」——但截圖實際是1920x1080，證實模型內部對圖片做了resize
- 進一步測試「整條工作列」的邊界框，答案的x2精確落在999——這是關鍵線索：**模型輸出的不是像素座標，是0~1000的正規化座標**（Qwen-VL系列訓練慣例），不管prompt裡怎麼講「請用像素座標」，模型還是照它訓練時習慣的方式回答
- **修正**：prompt明確要求0~1000正規化座標，程式碼自己換算成`(x/1000*圖片寬, y/1000*圖片高)`。修正後8B小模型（不用換30B）在兩個不同UI元素（開始按鈕、Chrome圖示）測試都準確定位（見`progress/p5_vision/evidence/fixed_grounding_*.png`）

**已刪除下載的30B模型檔案（19.3GB）**，不需要——原本方案B選的8B小模型完全夠用，卡住的從來不是模型能力，是我自己的prompt沒有講清楚座標系統慣例。這是一次值得記錄的教訓：VLM輸出「看起來合理但對不上」的座標時，應該先懷疑座標系統假設，不要先跳去假設模型能力不夠去找更大的模型——换大模型這次不但沒解決問題，還多花了下載19.3GB跟等待近30秒載入時間才發現真正原因。
